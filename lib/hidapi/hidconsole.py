...
def strhex(d): return hexlify(d).decode('ascii').upper()


try:
    unicode
    # this is certanly Python 2
    def is_string(d): return isinstance(d, unicode)
...
def _print(marker, data, scroll=False):
    ...
    if is_string(data):
        ...
        hexs = strhex(data)
        s = '%s (% 8.3f) [%s %s %s %s] %s' % (
            marker, t, hexs[0:2], hexs[2:4], hexs[4:8], hexs[8:], repr(data))
...
def _parse_arguments():
    ...
    arg_parser.add_argument('--hidpp', action='store_true',
...
def main():
    ...
    if interactive:
        ...
        if args.history is None:
            import os.path
            args.history = os.path.join(
                os.path.expanduser('~'), '.hidconsole-history')
        try:
