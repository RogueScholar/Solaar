...
def _look_for_application_icons():
    ...
    src_share = _path.normpath(_path.join(
        _path.realpath(_sys.path[0]), '..', 'share'))
    local_share = _environ.get('XDG_DATA_HOME', _path.expanduser(
        _path.join('~', '.local', 'share')))
    data_dirs = _environ.get('XDG_DATA_DIRS', '/usr/local/share:/usr/share')
    repo_share = _path.normpath(_path.join(
        _path.dirname(__file__), '..', '..', '..', 'share'))
    del _sys

    share_solaar = [prefix_share] + list(_path.join(x, 'solaar')
...
def _battery_icon_name(level, charging):
    ...
    if _has_mint_icons:
        ...
        level_name = ('empty', 'caution', 'low', 'good',
                      'good', 'full')[level_approx // 20]
    ...
    if _has_oxygen_icons:
        ...
        level_name = ('low', 'caution', '040', '060',
                      '080', '100')[level_approx // 20]
