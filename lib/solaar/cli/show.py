...
def _print_receiver(receiver):
    ...
    print('  Has', paired_count, 'paired device(s) out of a maximum of %d.' %
    ...
    if activity:
        ...
        activity_text = ', '.join(('%d=%d' % (d, a))
                                  for d, a in activity if a > 0)
...
def _print_device(dev):
    ...
    if dev.online:
        ...
        if battery is not None:
            ...
            if battery_voltage:
                (voltage, charging, charge_sts,
                 charge_lvl, charge_type) = battery_voltage
                print('     Battery: %smV, %s.' %
